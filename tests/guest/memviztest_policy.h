...
#define POLICY_PGSIZE 4096L
#define POLICY_PGROUNDUP(value) \
  (((value) + POLICY_PGSIZE - 1) & ~(POLICY_PGSIZE - 1))
...
policy_state_init(struct policy_state *state, int kind, int frame_count)
{
  ...
  for(int frame = 0; frame < POLICY_MAX_FRAMES; frame++)
    state->frames[frame].page = -1;
}
...
/** 将 CLOCK 引用状态压缩成页号位图；FIFO 仅将其作为观察信息。 */
static uint
 ...
 * 推进一次纯策略决策。
 *
 * FIFO 使用 hand 指向最早进入的页；CLOCK 清除引用位并继续扫描，直到遇到
 * 引用位为零的页。函数只返回 victim，真实换出由调用者执行。
 */
static void
 ...
 * 访问真实匿名页，并验证换入后的首尾签名。
 *
 * 首次访问负责物化 lazy 页；后续访问已换出页时，第一处读会触发内核 page-in。
 */
static void
...
/** 分配页对齐的 lazy 匿名区间，并返回恢复 break 所需增长量。 */
static char *
policy_reserve_pages(int page_count, uint64 *old_break, int *growth)
{
  *old_break = (uint64)sbrk(0);
  uint64 base = POLICY_PGROUNDUP(*old_break);
  uint64 end = base + (uint64)page_count * POLICY_PGSIZE;
...
/** 恢复实验前 break，并验证缩容释放了全部非驻留 slot。 */
static void
...
/**
 * 在 #145 的真实 swap 机制上执行确定性策略实验。
 *
 * 策略先选择 victim，随后 swapout() 完成写盘、PTE 迁移和物理页释放；再次访问
 * swapped 目标时由 page fault 恢复。计数和 slot 清理把两层连接成闭环。
 */
static void
           ...
           struct policy_result *result)
{
  ...
  for(int step = 0; step < access_count; step++){
    ...
    char *target = base + (uint64)accesses[step].page * POLICY_PGSIZE;
    ...
    if(trace->victim >= 0){
      char *victim = base + (uint64)trace->victim * POLICY_PGSIZE;
...
/** 验证相同输入下 FIFO 与 CLOCK 的第二个 victim 不同。 */
static void
policy_test_contrast(void)
{
  ...
  int count = sizeof(accesses) / sizeof(accesses[0]);

  policy_run("policy-contrast", POLICY_FIFO, 5, 3, accesses, count,
             &policy_fifo_result);
  policy_run("policy-contrast", POLICY_CLOCK, 5, 3, accesses, count,
             &policy_clock_result);
...
policy_test_locality(void)
{
  ...
  policy_run("working-set-fits", POLICY_FIFO, 3, 3, accesses, count,
             &policy_fit_result);
  policy_run("working-set-exceeds", POLICY_FIFO, 3, 2, accesses, count,
             &policy_pressure_result);
...
/** 重复 CLOCK 序列，验证决策确定且上一轮未泄漏 slot。 */
static void
policy_test_repeatability(void)
{
  ...
  policy_run("policy-repeat", POLICY_CLOCK, 5, 3, accesses, count,
             &policy_repeat_result);
