...
`user/` 与 `kernel/` 只保存被测实现。测试二进制仍生成到 `user/_<name>`，这是构建产物位置，不是源码所有权；这样可以保持 `mkfs` 输入格式和 xv6 shell 中的命令名不变。
...
| Lab7 threads | `xv6test --group lab7` | `uthreadtest.c`；被测实现仍为 `user/uthread.c` | `ph`、`barrier` host 测试 |
...
1. 把测试源码放入 `tests/guest/`，不要放入 `user/` 或 `kernel/`。
