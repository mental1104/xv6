...
description: 在 mental1104/xv6 中新增、迁移、整理或维护测试时使用。适用于“新增测试”“补测试”“迁移到 xv6test”“调整 make test”“修改 tests/run.py”“为某个 Lab 增加回归”等请求。要求所有测试源码位于 tests/，user/、kernel/ 与 notxv6/ 只保存被测实现或非测试示例；优先让 xv6 guest 持有测试语义，Python 只负责 QEMU 编排、超时和日志；禁止新增或修改 GitHub Actions workflow；所有持久化改动必须通过 Issue、分支和 Draft PR。
---
...
不得在 `user/`、`kernel/` 或 `notxv6/` 新增 `*test.c`、测试专用脚本或 runner。编译后生成在 `user/_<name>` 的文件是构建产物，不代表源码所有权。
...
适合系统调用、VM、进程、文件系统和锁的行为测试。

### B. 已有独立测试程序

例如 `lazytests`、`cowtest`、`mmaptest`。
...
不要把测试断言写回被测命令，也不要复制实现函数到测试文件。

需要捕获 stdout/stderr 时优先复用：

```text
tests/guest/testlib.h
tests/guest/testlib.c
```

### D. 内核控制台输出无法由 guest fd 捕获

例如 backtrace、trace 控制台行。

```text
guest 测试先验证操作成功并 exit(0)
→ xv6test 汇总 status=0
→ Python 对同一 group 原始日志补充最小硬件/控制台观察
```

Host 检查只能验证 guest 无法捕获的观测，不得重新拥有完整业务语义。
...
源码放在 `tests/host/`，由 `tests/run.py` 编排和断言。不要放在 `notxv6/`，也不要伪装成 xv6 guest 测试。

### G. 性能、竞争或调试观察

吞吐量、锁竞争计数、GDB 跟踪、概率时序不应直接成为稳定 `make test` 阻塞项，除非已有明确阈值、固定环境和重复性证据。
...
Makefile 会把 `tests/guest/foo.c` 编译成：

```text
user/_foo
```

原因是 `mkfs` 仍使用 `user/` 一级路径和 `_` 前缀生成 guest 根目录命令。不得因为源码分离而改变 guest 命令名。
...
通用规则会从 `$T/<name>.o` 链接到稳定的 `$U/_<name>`。
...
```text
group: lab1 ... lab10、core、legacy、regression
name:  <group>-<behavior>
```

名称必须全局唯一且稳定，禁止把临时 Issue 编号写入测试名。
...
```python
TestCase(..., commands=("usertests case",), expected=("业务输出",))
```
...
- suite 引用存在；
- QEMU Lab 命令全部经 `xv6test`；
- 测试源码只位于 `tests/guest/`；
- 新测试不与其他 suite 重复；
- 特殊控制台检查仍要求 `XV6TEST done status=0`；
- README coverage map 已更新。

---

## 输出协议

`xv6test` 协议属于 host/guest 稳定边界：
...
- 父进程必须准确回收目标 PID；
- 任何非零退出状态必须成为 `not ok`；
- Python 只以 `XV6TEST done status=0` 判断 guest 汇总成功。
...
`CPUS=1` 只用于诊断；不得用单核通过证明多核正确。

### 磁盘破坏型测试

确认 host suite 使用独立 snapshot，并检查后续测试不受前序磁盘写入影响。
...
- 新增 `.github/workflows/*.yml` / `*.yaml`；
- 修改现有 workflow；
- 依赖 GitHub Actions 作为唯一验证证据；
- 私自把 Draft PR 标记为 ready。

用户明确要求本地验收时，只提交代码、测试、文档和本地命令。
