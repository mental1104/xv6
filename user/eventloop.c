 ...
 * 返回事件源的稳定公开名称。
 *
 * @param source SOURCE_A 或 SOURCE_B。
 * @return 对应的静态字符串；非法来源返回问号。
 */
static char *
 ...
 *
 * @param fd 当前子进程独占的 pipe 写端。
 * @param source 事件源编号。
 * @param sequence 该事件源内从 1 开始的顺序号。
 */
static void
 ...
 * 生成固定、可重复的多源事件负载。
 *
 * @param source 当前子进程拥有的事件源编号。
 * @param fd 当前子进程独占的 pipe 写端；函数结束前关闭。
 *
 ...
 *
 * @param source 子进程负责的事件源编号。
 * @param pipes 两个事件源的 pipe；子进程只保留自己的写端。
 * @param gate 父进程完成非阻塞快照后用于同时释放两个来源的 pipe。
 * @return 新建子进程 PID；基础设施失败时终止父进程。
 */
static int
 ...
 *
 * @param states 两个事件源的权威状态。
 * @param fds 接收仍未 EOF 的读端数组。
 * @param indexes 接收 fds 每个槽位对应的 states 下标。
 * @return 本轮有效事件源数量。
 */
static int
 ...
 *
 * @param states 两个来源的权威状态表。
 * @param state_index 当前事件所属状态下标。
 * @param event 已完整读取的事件值。
 * @param slow_handler 是否让 A 的首个处理器故意阻塞事件循环。
 * @param total_handled 接收并累加全局已处理事件数。
 * @param slow_source_lag 接收慢模式下 B 从产生到分派的等待 tick。
 *
 ...
 * 执行一次完整的多源事件循环实验。
 *
 * @param slow_handler 非零时让 A 的首个 handler 阻塞 20 tick。
 * @return 所有状态、EOF、子进程和慢路径 oracle 均成立时返回 0。
 */
static int
 ...
 *
 * @param argc 参数数量，只允许无参数或一个 --slow。
 * @param argv 参数数组。
 * @return 实验成功返回 0；参数错误返回 2；不变量失败由 fail() 返回 1。
 */
int
